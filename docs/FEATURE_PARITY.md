...
| 搜索默认页、热搜、历史、建议 | `components/Search/*` | 未实现 | 防抖、历史、建议与热搜测试 |
| 搜索歌曲 | `views/Search/songs.vue` | 部分 | 分页、筛选、播放、菜单 |
| 搜索歌单、歌手、专辑、视频、播客 | `views/Search/*` | 未实现 | 六类结果和详情跳转 |
...
| 歌单详情 | `views/List/playlist.vue` | 部分 | 当前歌单仅列出，详情不可进入 |
| 歌曲百科、乐谱、回忆坐标 | `views/Song/wiki.vue` | 未实现 | 数据加载与异常态 |
| 每日推荐 | `views/DailySongs.vue` | 部分 | 需登录态和“不感兴趣” |
| 喜欢的歌曲 | `views/List/liked.vue` | 未实现 | 喜欢/取消与实时同步 |
| 收藏：歌单、专辑、歌手、视频、播客 | `views/Like/*` | 未实现 | 五类收藏完整显示与操作 |
...
| 评论列表 | `views/Comment.vue` | 部分 | 当前仅首屏文本，无分页/排序/楼中楼 |
| 评论点赞、抱一抱、回复 | `api/comment.ts` | 未实现 | 登录校验与状态回写 |
...
| 上一首、下一首、播放队列 | `SongManager.ts` | 部分 | 缺队列编辑、拖动、插播、移除 |
